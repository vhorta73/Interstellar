...
namespace Interstellar::Renderers::OpenGL {
    ...
    bool OpenGLShader::LoadAndCompile(const std::string& vertexPath, const std::string& fragmentPath) {
        ...
        if (vertexCode.empty() || fragmentCode.empty()) {
            logGraphic.LogError("Shader source is empty (vertex: '{}', fragment: '{}')", vertexPath, fragmentPath);
            return false;
        }
        ...
        if (!success) {
            ...
            logGraphic.LogError("Shader program link failed for '{}': {}", m_Name, log);
        ...
        logGraphic.LogDebug("Shader '{}' compiled and linked successfully.", m_Name);
        return true;
    }
    ...
    unsigned int OpenGLShader::CompileShader(unsigned int type, const std::string& source) {
        ...
        if (!success) {
            ...
            logGraphic.LogError("{} shader compilation failed for '{}': {}", typeStr, m_Name, log);
    ...
    std::string OpenGLShader::LoadFile(const std::string& path) {
        ...
        if (!file.is_open()) {
            logGraphic.LogError("Failed to open shader file: {}", path);
            return {};
        }
