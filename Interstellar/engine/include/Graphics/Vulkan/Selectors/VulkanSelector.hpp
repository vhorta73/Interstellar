...
namespace Graphics::Vulkan::Selectors {
    //
    // Your preferred hard-coded combos:
    // for each layer, a ranked list of extensions;
    // for each extension, a ranked list of layers.
    //
    static const std::vector<std::pair<const char*, std::vector<const char*>>> PreferredLayerExtensionCombos = {
    ...
    static const std::vector<std::pair<const char*, std::vector<const char*>>> PreferredExtensionLayerCombos = {
        { "VK_EXT_debug_utils",
            {
                "VK_LAYER_KHRONOS_validation",
            }
