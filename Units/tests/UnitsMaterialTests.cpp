...
namespace UnitsMaterialsTests {

    TEST(UnitsMaterialsTests, coefficients_are_unit_free_scalars) {
    ...
    TEST(UnitsMaterialsTests, friction_force_has_newton_units_and_correct_magnitude) {
        ...
        EXPECT_NEAR_Q(F_f, U::N, 0.5 * 80.0 * 9.80665, 1e-9);
    }

    TEST(UnitsMaterialsTests, drag_force_example_is_in_newtons) {
    ...
    TEST(UnitsMaterials, albedo_affects_equilibrium_temperature_monotonically) {
        ...
        EXPECT_NEAR(thermo::in_K(T_A03), 254.0, 1.0);

        // Brighter A = 0.6 -> cooler
        const auto T_A06 = equilibrium_temp(S_flux, 0.6);
        EXPECT_LT(thermo::in_K(T_A06), thermo::in_K(T_A03));
        EXPECT_LT(thermo::in_K(T_A03), thermo::in_K(T_A0));
    }
