...
namespace ConfigStorageTests {
    ...
    public:
        ...
        {
            cfg = AppConfig::New(&storage, "root");
        }
        ...
        TEST_METHOD(TestSaveIntent)
        {
            ...
            Assert::AreEqual(std::string("root/" + dir), storage.dirsCreated[0]);

            // Assert file path
            Assert::AreEqual(
                std::string("root/"+dir+"/"+fileName),
        ...
        TEST_METHOD(TestLoadIntent)
        {
            ...
            std::string path = "root/user/info.json";
